...
# Load ElevenLabs API key from 1Password if not already set
if [[ -z "$ELEVENLABS_API_KEY" ]]; then
  export ELEVENLABS_API_KEY=$(op run --env-file=/Users/kieransteele/git/envs/mcps_1p.env -- bash -c 'echo $L1_elevenlabs_apikey' 2>/dev/null)
fi
